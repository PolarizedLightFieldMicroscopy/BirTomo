...
def recon_sphere():
    ...
    if simulate:
        ...
        if True:
            ...
            np.save("data/forward_images/ret_" + foward_img_str, ret_numpy)
            azim_numpy = azim_image_meas.detach().numpy()
            np.save("data/forward_images/azim_" + foward_img_str, azim_numpy)
    else:
        ret_image_meas = np.load(
            os.path.join("data/forward_images", "ret_" + foward_img_str)
        )
        azim_image_meas = np.load(
            os.path.join("data/forward_images", "azim_" + foward_img_str)
        )
...
def recon_sphere6_thick():
    ...
    recon_directory = create_unique_directory(
        "reconstructions", postfix="sphere6_thick2"
    )
    if not simulate:
        volume_GT = initial_volume
    recon_config = ReconstructionConfig(
