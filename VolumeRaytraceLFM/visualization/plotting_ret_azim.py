...
def plot_birefringence_lines(retardance_img, azimuth_img, origin='lower', upscale=1, cmap='Wistia_r', line_color='blue', ax=None):
    ...
    lc_data = [[(l_ii[ix], l_jj[ix]), (h_ii[ix], h_jj[ix])]
               for ix in range(len(l_ii))]
    colors = retardance_img.flatten()
    cmap = matplotlib.cm.get_cmap(cmap)
    rgba = cmap(colors/(2*np.pi))

    lc = matplotlib.collections.LineCollection(
        lc_data, colors=line_color, linewidths=1)
    if ax is None:
        fig, ax = plt.subplots()
...
def plot_hue_map(retardance_img, azimuth_img, ax=None):
    ...
    cb1 = plt.colorbar(plt.cm.ScalarMappable(cmap=cmap_hue),
    ...
    cmap_grey = LinearSegmentedColormap.from_list('custom_gray',
                                                  [(0, 0, 0), (1, 1, 1)], N=256)
    cb2 = plt.colorbar(plt.cm.ScalarMappable(cmap=cmap_grey),
                       cax=axins2, orientation="vertical")
