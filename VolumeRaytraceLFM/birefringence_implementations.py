...
class BirefringentRaytraceLFM(RayTraceLFM, BirefringentElement):
    ...
    def print_timing_info(self, precision=2, unit='ms'):
        ...
        print("Generating MLA images (sum of indiv lenslets times):", fmt_str.format(sum(self.mla_execution_times.values()) * multiplier), unit_str)
    ...
    def ray_trace_through_volume(self, volume_in : BirefringentVolume = None,
        ...
        if all_rays_at_once != self.MLA_volume_geometry_ready:
            raise ValueError("The geometry for the MLA is not prepared appropriately "
        ...
        if all_rays_at_once:
            if intensity:
                raise NotImplementedError("Intensity images are not supported for all rays at once.")
        ...
        try:
            for m in range(len(voxels_of_segs[n_ray])):
                ...
                opticAxis = volume_in.optic_axis[:,
                                                ...
                                                vox[1]+microlens_offset[0],
                                                vox[2]+microlens_offset[1]]
                jones = self.voxRayJM(Delta_n, opticAxis, rayDir, ell, self.optical_info['wavelength'])
    ...
    def calc_cummulative_JM_of_ray_torch(self, volume_in : BirefringentVolume,
        ...
        for m in range(ell_in_voxels.shape[1]):
            ...
            try:
                ...
                jones = self.voxRayJM(Delta_n=Delta_n, opticAxis=opticAxis,
                ...
                self.times['jones_matrix_multiplication'] += end_time_jones_mult - start_time_jones_mult
            except IndexError as e:
                err_msg = ("Cumulative Jones Matrix computation failed. " +
    ...
    def vox_ray_ret_azim(self, Delta_n, opticAxis, rayDir, ell, wavelength):
        ...
        start_time = time.perf_counter()
        if self.backend == BackEnds.NUMPY:
            ret, azim = jones_matrix.vox_ray_ret_azim_numpy(Delta_n, opticAxis, rayDir, ell, wavelength)
        else:
            if DEBUG:
