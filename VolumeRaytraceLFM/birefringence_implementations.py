...
class BirefringentRaytraceLFM(RayTraceLFM, BirefringentElement):
    ...
    def ray_trace_through_volume(self, volume_in : BirefringentVolume = None, all_rays_at_once=True, intensity=False):
        ...
        for ml_ii in tqdm(range(-n_ml_half, n_ml_half+odd_mla_shift), f'Computing rows of micro-lens ret+azim {self.backend}'):
            ...
            for ml_jj in range(-n_ml_half, n_ml_half+odd_mla_shift):
                ...
                if intensity == True:
                    ret_image_torch, azim_image_torch = self.intensity_images_numpy_og(volume_in, micro_lens_offset=current_offset)
                else:
    ...
    def ray_trace_through_volume_intensity(self, volume_in : BirefringentVolume = None, all_rays_at_once=True):
        """ This function forward projects a whole volume, by iterating through the volume in front of each micro-lens in the system.
            By computing an offset (current_offset) that shifts the volume indices reached by each ray.
            Then we accumulate the images generated by each micro-lens, and concatenate in a final image"""
        ...
        # TODO: eliminate old notation of full_img_r, etc
        # Traverse volume for every ray, and generate intensity images
        full_img_r = None
        full_img_a = None
        ...
        for ml_ii in tqdm(range(-n_ml_half, n_ml_half+odd_mla_shift), f'Computing rows of micro-lens ret+azim {self.backend}'):
            ...
            for ml_jj in range(-n_ml_half, n_ml_half+odd_mla_shift):
                ...
                # Compute images for current microlens, by passing an offset to this function depending on the micro lens and the super resolution
                intensity_image_list = self.intensity_images(volume_in, micro_lens_offset=current_offset)
                ret_image_torch, azim_image_torch = intensity_image_list[1], intensity_image_list[2]
                # If this is the first image, create
                if full_img_row_list[0] is None:
                    full_img_row_list = intensity_image_list
                    full_img_row_r = ret_image_torch
                    full_img_row_a = azim_image_torch
                else: # Concatenate to existing image otherwise
                    if self.backend == BackEnds.NUMPY:
                        full_img_row_list = [np.concatenate((img0, img1), 0) for img0, img1 in zip(full_img_row_list, intensity_image_list)]
                        full_img_row_r = np.concatenate((full_img_row_r, ret_image_torch), 0)
                        full_img_row_a = np.concatenate((full_img_row_a, azim_image_torch), 0)
                    elif self.backend == BackEnds.PYTORCH:
                        full_img_row_list = [torch.cat((img0, img1), 0) for img0, img1 in zip(full_img_row_list, intensity_image_list)]
                        full_img_row_r = torch.cat((full_img_row_r, ret_image_torch), 0)
                        full_img_row_a = torch.cat((full_img_row_a, azim_image_torch), 0)
    ...
    def intensity_images_numpy_og(self, volume_in : BirefringentVolume, micro_lens_offset=[0,0]):
        '''Calculate intensity values for a ray with a Jones Matrix'''
            ...
            for j in range(pixels_per_ml):
                if np.isnan(self.ray_entry[0, i, j]):
                    ...
                    for setting in range(4):
                        polarizer = JonesMatrixGenerators.universal_compensator_modes(setting=setting, swing=0.03) @ JonesVectorGenerators.horizonal()
                    E_out = analyzer @ effective_JM @ JonesMatrixGenerators.universal_compensator_modes(setting=1, swing=0.3) @ JonesVectorGenerators.horizonal()
                    intensity = np.linalg.norm(E_out) ** 2
                    ret_image[i, j] = intensity
                    azim_image[i, j] = self.intensity_JM(effective_JM)
                    # if np.isclose(ret_image[i, j], 0.0):
                    #     azim_image[i, j] = 0
                    # else:
                    #     azim_image[i, j] = self.azimuth(effective_JM)
    ...
    def intensity_images_numpy(self, volume_in : BirefringentVolume, micro_lens_offset=[0,0]):
        '''Calculate intensity images using Jones Calculus.
        The analyzer is applied with the rest of the voxels.
        Here the polarizer is applied.'''
    ...
    def calc_cummulative_JM_lenslet_numpy(self, volume_in : BirefringentVolume, micro_lens_offset=[0,0]):
        ...
        for i in range(pixels_per_ml):
            for j in range(pixels_per_ml):
                if np.isnan(self.ray_entry[0, i, j]):
                    ...
                    lenslet[i, j, :, :] = self.calc_cummulative_JM_of_ray_numpy(i, j, volume_in, micro_lens_offset)
        return lenslet      
