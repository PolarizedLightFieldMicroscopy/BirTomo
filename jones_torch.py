...
def voxRayJM(Delta_n, opticAxis, rayDir, ell):
    ...
    if not torch.is_tensor(opticAxis):
        opticAxis = torch.from_numpy(opticAxis).double()
...
def rayJM(JMlist):
    ...
    '''
    product = torch.tensor([[1.0,0],[0,1.0]], dtype=torch.complex128)
    for JM in JMlist:
        product = product @ JM
    return product
...
def calc_rayDir(ray_in):
    ...
    if not torch.is_tensor(ray_in):
        ray = torch.from_numpy(ray_in).double()
    else:
        ray = ray_in
    theta = torch.arccos(torch.dot(ray, torch.tensor([1.0,0,0])))
    # Unit vectors that give the laboratory axes, can be changed
    scope_axis = torch.tensor([1.0,0,0])
    scope_perp1 = torch.tensor([0,1.0,0])
    scope_perp2 = torch.tensor([0,0,1.0])
    theta = torch.arccos(torch.dot(ray, scope_axis))
    print(f"Rotating by {np.around(torch.rad2deg(theta).numpy(), decimals=0)} degrees")
    normal_vec = torch.cross(ray, scope_axis) / torch.linalg.norm(torch.cross(ray, scope_axis))
...
def calc_retardance(JM):
    ...
    # print(f"sqrt portion: {torch.sqrt((off_diag_sum / diag_sum) ** 2 + (diag_diff / diag_sum) ** 2)}")
    # Note: torch.arctan(1j) and torch.arctan(-1j) gives an divide by zero error
