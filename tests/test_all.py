...
def test_rays_computation(global_data, pixels_per_ml_init):
    ...
    BF_raytrace_numpy.ray_direction[np.isnan(
        BF_raytrace_numpy.ray_direction)] = -10
    BF_raytrace_torch.ray_direction[torch.isnan(
        BF_raytrace_torch.ray_direction)] = -10
...
def test_voxel_array_creation(global_data, iteration):
    ...
    voxel_torch = BirefringentVolume(backend=BackEnds.PYTORCH, optical_info=optical_info,
                                     ...
                                     optic_axis=torch.tensor(optic_axis).unsqueeze(1).unsqueeze(1).unsqueeze(
                                         1).repeat(1, volume_shape[0], volume_shape[1], volume_shape[2])
                                     )

    # Passing an already build 3D array
    voxel_numpy = BirefringentVolume(backend=BackEnds.NUMPY, optical_info=optical_info,
                                     Delta_n=delta_n *
                                     torch.ones(volume_shape).numpy(),
                                     optic_axis=torch.tensor(optic_axis).unsqueeze(1).unsqueeze(1).unsqueeze(
                                         1).repeat(1, volume_shape[0], volume_shape[1], volume_shape[2]).numpy()
                                     )
    voxel_torch = BirefringentVolume(backend=BackEnds.PYTORCH, optical_info=optical_info,
    ...
    assert np.all(np.isclose(voxel_numpy.optic_axis.flatten(), voxel_torch.optic_axis.detach().numpy(
    ).flatten())), f"Optic axis mismatch between numpy/pytorch delta_n: {delta_n}, optic_axis: {optic_axis}"
    ...
    assert np.all(np.isclose(voxel_numpy_single_value.optic_axis.flatten(), voxel_torch_single_value.optic_axis.detach(
    ).numpy().flatten())), f"Optic axis mismatch between numpy/pytorch delta_n: {delta_n}, optic_axis: {optic_axis}"
    assert np.all(np.isclose(voxel_numpy_single_value.Delta_n.flatten(), voxel_torch_single_value.Delta_n.detach(
    ).numpy().flatten())), f"Delta_n mismatch between numpy/pytorch delta_n: {delta_n}, optic_axis: {optic_axis}"
...
def test_compute_JonesMatrices(global_data, volume_shape_in):
    ...
    for ii in range(pixels_per_ml):
        for jj in range(pixels_per_ml):
            ...
            ret_numpy, azi_numpy = BF_raytrace_numpy.retardance(
                JM_numpy), BF_raytrace_numpy.azimuth(JM_numpy)
    ...
    ret_torch, azi_torch = BF_raytrace_torch.retardance(
        JM_torch), BF_raytrace_torch.azimuth(JM_torch)
    ...
    for ray_ix in range(BF_raytrace_torch.ray_valid_indices.shape[1]):
        ...
        assert np.isclose(JM_numpy.astype(np.complex64), JM_torch[ray_ix].detach(
        ).numpy(), atol=1e-5).all(), f'JM mismatch on coord: (i,j)= ({i},{j}):'
        ...
        try:
            ...
            assert np.isclose(ret_img_numpy[i, j], ret_img_torch[i, j], atol=1e-5).all(
            ), f'Retardance mismatch on coord: (i,j)= ({i},{j}):'
        except:
            print(f'Retardance mismatch on coord: (i,j)= ({i},{j}):')
            any_fail = True
        try:
            # Check azimuth for this ray
            check_azimuth_images(np.array([azi_img_numpy[i, j]]), np.array(
                azi_img_torch[i, j]), message=f'Azimuth mismatch on coord: (i,j)=({i},{j}):')
        except:
...
def test_compute_retardance_and_azimuth_images(global_data, iteration):
    ...
    [ret_img_numpy, azi_img_numpy] = BF_raytrace_numpy.ret_and_azim_images(
        voxel_numpy)
    [ret_img_torch, azi_img_torch] = BF_raytrace_torch.ret_and_azim_images(
        voxel_torch)
    # Use this in debug console to visualize errors
    # plot_ret_azi_image_comparison(ret_img_numpy, azi_img_numpy, ret_img_torch, azi_img_torch)

    assert np.all(np.isnan(ret_img_numpy) ==
                  False), "Error in numpy retardance computations nan found"
    assert np.all(np.isnan(azi_img_numpy) ==
                  False), "Error in numpy azimuth computations nan found"
    assert torch.all(torch.isnan(ret_img_torch) ==
                     False), "Error in torch retardance computations nan found"
    assert torch.all(torch.isnan(azi_img_torch) ==
...
def test_azimuth_neg_birefringence(global_data):
    ...
    voxel_pos_torch = BirefringentVolume(
        backend=BackEnds.PYTORCH, optical_info=optical_info, volume_creation_args=voxel_args_pos)
    voxel_neg_torch = BirefringentVolume(
        backend=BackEnds.PYTORCH, optical_info=optical_info, volume_creation_args=voxel_args_neg)
    voxel_pos_numpy = BirefringentVolume(
        backend=BackEnds.NUMPY, optical_info=optical_info, volume_creation_args=voxel_args_pos)
    voxel_neg_numpy = BirefringentVolume(
        backend=BackEnds.NUMPY, optical_info=optical_info, volume_creation_args=voxel_args_neg)
    ...
    plot_ret_azi_flipsign_image_comparison(ret_img_pos.numpy(
    ), azi_img_pos.numpy(), ret_img_neg.numpy(), rotated_azi_img_neg.numpy())
    assert np.all(np.isclose(ret_img_pos.numpy(), ret_img_neg.numpy(
    ), atol=1e-5)), "Retardance depends on the sign of the birefrigence."
    check_azimuth_images(azi_img_pos.numpy(), rotated_azi_img_neg.numpy(
    ...
    assert np.all(np.isclose(ret_img_pos.astype(np.float32), ret_img_neg.astype(
        np.float32), atol=1e-5)), "Retardance depends on the sign of the birefrigence."
    check_azimuth_images(azi_img_pos.astype(np.float32), rotated_azi_img_neg.astype(
        np.float32), "Flipping the sign of the birefringence does not simply rotate the azimuth.")
...
def test_intensity_with_both_methods(global_data):
    ...
    for backend in backends:
        ...
        rays = BirefringentRaytraceLFM(
            backend=backend, optical_info=optical_info)
...
def check_azimuth_images(img1, img2, message="Error when comparing azimuth computations"):
    ...
    if not np.all(np.isclose(img1, img2, atol=1e-5)):
        ...
        assert np.all(np.isclose(
            diff[~np.isclose(diff, 0.0, atol=1e-5)], np.pi, atol=1e-5)), message
