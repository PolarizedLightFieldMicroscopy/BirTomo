...
def test_get_delta_n(optical_info, backend):
    bv = BirefringentVolume(
        backend=backend, optical_info=optical_info, Delta_n=0.2)
...
def test_get_optic_axis(optical_info, backend):
    bv = BirefringentVolume(
        backend=backend, optical_info=optical_info, optic_axis=[1.0, 0.0, 0.0])
    ...
    assert np.array_equal(optic_axis[:, idx[0], idx[1], idx[2]], [
                          1.0, 0.0, 0.0]), f"Expected optic_axis to be [1.0, 0.0, 0.0], but got {optic_axis[:, idx[0], idx[1], idx[2]]}"
...
def test_get_vox_params(optical_info, backend):
    ...
    assert np.array_equal(optic_axis, [
                          1.0, 0.0, 0.0]), f"Expected optic_axis to be [1.0, 0.0, 0.0], but got {optic_axis}"
...
def test_init_from_file(optical_info, tmp_path):
    ...
    bv = BirefringentVolume.init_from_file(
        file_path, backend=BackEnds.NUMPY, optical_info=optical_info)
