...
def test_get_delta_n(optical_info_vol11, backend_fixture):
    ...
    if backend_fixture == BackEnds.PYTORCH:
        delta_n = (
            delta_n.detach().cpu().numpy()
            if delta_n.requires_grad
            else delta_n.cpu().numpy()
        )
    assert np.isclose(
        delta_n.max(), 0.2
    ), f"Expected maximum delta_n to be 0.2, but got {delta_n.max()}"
...
def test_get_optic_axis(optical_info_vol11, backend_fixture):
    ...
    if backend_fixture == BackEnds.PYTORCH:
        optic_axis = (
            optic_axis.detach().cpu().numpy()
            if optic_axis.requires_grad
            else optic_axis.cpu().numpy()
        )
    ...
    assert np.array_equal(
        optic_axis[:, idx[0], idx[1], idx[2]], [1.0, 0.0, 0.0]
    ), err_message
