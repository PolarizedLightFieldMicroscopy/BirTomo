...
def test_identify_voxels_repeated_zero_ret_empty_list(backend_fixture):
    ...
    assert (
        len(counts) == vol_shape[0]
    ), f"Expected {vol_shape[0]} voxels raytraced with volume"
    assert (
        len(vox_list) == 0
    ), "Expected zero voxels raytraced excluding repeated zero retardance voxels"
    ...
    if test_3x3mla:
        ...
        vox_list_err_msg = (
            "Expected 71 voxels raytraced excluding repeated "
    ...
    err_msg = (
        "Expected 198 voxels raytraced excluding repeated "
        + f"zero retardance voxels. Got: {vox_list}"
    )
