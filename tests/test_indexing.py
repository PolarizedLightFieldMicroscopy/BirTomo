...
def test_forward_projection_exception_raising(backend_fixture):
    ...
    optical_info = set_optical_info([7, 7, 7], 17, 3)
    optical_info['n_medium'] = 1.52

    BF_raytrace = BirefringentRaytraceLFM(backend=backend_fixture, optical_info=optical_info)
    BF_raytrace.compute_rays_geometry()

    voxel_volume = BirefringentVolume(backend=backend_fixture,
        optical_info=optical_info, volume_creation_args={'init_mode': 'single_voxel'})
