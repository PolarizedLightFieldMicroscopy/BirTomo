...
@pytest.mark.parametrize(
    "volume_init_mode", ["random", "ellipsoid", "1planes", "3planes"]
)
...
def test_torch_auto_differentiation(global_data, volume_init_mode):
    ...
    assert (
        volume_torch_random.Delta_n.grad.sum() != 0
    ), "Gradients were not propagated to the volumes Delta_n correctly"
    assert (
        volume_torch_random.optic_axis.grad.sum() != 0
    ...
    ), "Seems like the volume wasnt updated correctly, nothing changed in the volume after the optimization step"
    assert (
