...
def test_forward_projection_lenslet_grid_random_volumes(global_data, volume_shape_in):
    ...
    voxel_torch_random = BirefringentVolume(
        backend=BackEnds.PYTORCH, optical_info=optical_info, volume_creation_args={'init_mode': 'random'})
...
def test_torch_auto_differentiation(global_data, volume_init_mode):
    ...
    assert volume_torch_random.Delta_n.grad.sum(
    ) != 0, 'Gradients were not propagated to the volumes Delta_n correctly'
    assert volume_torch_random.optic_axis.grad.sum(
    ) != 0, 'Gradients were not propagated to the volumes optic_axis correctly'
...
def plot_azimuth(img):
    ...
    cbar = fig.colorbar(cax, ticks=np.rad2deg(
        [-np.pi, -np.pi/2, -np.pi/3, -np.pi/4, -np.pi/6, 0, np.pi/6, np.pi/4, np.pi/3, np.pi/2, np.pi]))
