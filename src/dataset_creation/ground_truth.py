...
def single_shell():
    my_volume = BirefringentVolume.create_dummy_volume(
        backend=backend,
    ...
    plotly_figure = my_volume.plot_volume_plotly(
        optical_info, voxels_in=my_volume.get_delta_n(), opacity=0.01, fig=plotly_figure
    )
    plotly_figure.show()
...
def single_ellipse(rays, i):
    ellip_volume = BirefringentVolume.create_dummy_volume(
        backend=BackEnds.NUMPY,
        optical_info=optical_info,
    ...
    if plot_volume:
        ...
        plotly_figure = ellip_volume.plot_volume_plotly(
            optical_info,
            voxels_in=ellip_volume.get_delta_n(),
            opacity=0.01,
            fig=plotly_figure,
        )
        plotly_figure.show()
