...
if st.button("Plot volume!"):
    ...
    my_fig = st.session_state["my_volume"].plot_volume_plotly(
        optical_info, voxels_in=st.session_state["my_volume"].Delta_n, opacity=0.1
    )
...
def forward_propagate():
    ...
    try:
        ...
        execution_time = time.time() - start_time
        st.text(
            f"Execution time in seconds with backend {backend}: " + str(execution_time)
        )
...
def forward_propagate_intensity():
    ...
    try:
        ...
        start_time = time.time()
        my_image_list = rays.ray_trace_through_volume(
            st.session_state["my_volume"], intensity=True
        )
        execution_time = time.time() - start_time
        st.text(
            f"Execution time in seconds with backend {backend}: " + str(execution_time)
...
save_directory = st.text_input(
    "Enter the directory path to save the images", value=default_directory
)
...
st.markdown(
    "Provide information to save with the images as part of the **TIF metadata**. "
...
if backend == BackEnds.PYTORCH:
    st.error(
        "Use the numpy backend for the intensity images. "
        + "The pytorch backend is not fully implemented yet."
    )
else:
    optical_info["polarizer_swing"] = st.slider(
        "Swing retardance", min_value=0.01, max_value=0.1, value=0.03
    )
