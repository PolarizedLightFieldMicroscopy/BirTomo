...
with columns[0]:
    ...
    optical_info["n_micro_lenses"] = st.slider(
        "Number of microlenses", min_value=1, max_value=51, value=9
    )
    ...
    optical_info["n_voxels_per_ml"] = st.slider(
        "Number of voxels per microlens (volume sampling)",
        min_value=1,
        max_value=7,
        value=1,
    )
    ...
    regularization_function1 = st.selectbox(
        "Volume regularization function 1", ["L1", "L2", "unit", "TV", "none"], 2
    )
    regularization_function2 = st.selectbox(
        "Volume regularization function 2", ["L1", "L2", "unit", "TV", "none"], 4
    )
    reg_weight1 = st.number_input(
        "Regularization weight 1", min_value=0.0, max_value=0.5, value=0.5
    )
    # st.write('The current regularization weight 1 is ', reg_weight1)
    reg_weight2 = st.number_input(
        "Regularization weight 2", min_value=0.0, max_value=0.5, value=0.5
    )
    # st.write('The current regularization weight 2 is ', reg_weight2)
    ret_azim_weight = st.number_input(
        "Retardance-Orientation weight", min_value=0.0, max_value=1.0, value=0.5
    )
    # st.write('The current retardance/orientation weight is ', ret_azim_weight)
    st.subheader("Initial estimated volume")
    volume_init_type = st.selectbox("Initial volume type", ["random", "upload"], 0)
    if volume_init_type == "upload":
        ...
        delta_n_init_magnitude = st.number_input(
            "Volume Delta_n initial magnitude",
            min_value=0.0,
            max_value=1.0,
            value=0.0001,
            format="%0.5f",
    ...
    learning_rate_delta_n = st.number_input(
        "Learning rate for Delta_n",
        min_value=0.0,
        max_value=10.0,
        value=0.001,
        format="%0.5f",
    ...
    learning_rate_optic_axis = st.number_input(
        "Learning rate for optic_axis",
        min_value=0.0,
        max_value=10.0,
        value=0.001,
        format="%0.5f",
...
with columns[1]:
    ...
    with volume_container:
        how_get_vol = st.radio(
            ...
            ["h5 upload", "Create a new volume", "Upload experimental images"],
            index=1,
        )
        if how_get_vol == "h5 upload":
            h5file = st.file_uploader("Upload Volume h5 Here", type=["h5"])
            optical_info["n_voxels_per_ml_volume"] = st.slider(
                "Number of voxels per microlens in volume",
                min_value=1,
                max_value=21,
                value=1,
            )
            if h5file is not None:
                ...
                st.markdown(
                    f"Using a cube volume shape with the dimension of the"
            ...
            retardance_path = st.file_uploader(
                "Upload retardance tif", type=["png", "tif", "tiff"]
            )
            azimuth_path = st.file_uploader(
                "Upload orientation tif", type=["png", "tif", "tiff"]
            )
            metadata_file = st.file_uploader(
                "Upload metadata from Napari-LF", type=["txt"]
            )
            plot_cropped_imgs = st.empty()  # set up a place holder for the plot

            # Load files
            if retardance_path is not None:
                ret_img_raw = torch.from_numpy(
                    np.array(Image.open(retardance_path)).astype(np.float32)
                )
            if azimuth_path is not None:
                azim_img_raw = torch.from_numpy(
                    np.array(Image.open(azimuth_path)).astype(np.float32)
                )
            ...
            if metadata_file is not None:
                ...
                optical_info["pixels_per_ml"] = (
                    metadata["calibrate"]["pixels_per_ml"]
                    if "pixels_per_ml" in metadata["calibrate"].keys()
                    else optical_info["pixels_per_ml"]
                )
            ...
            if retardance_path and azimuth_path and metadata_file:
                ...
                crop_pos_y = st.slider(
                    "Image region center Y", min_value=1, max_value=n_mls_y, value=55
                )
                crop_pos_x = st.slider(
                    "Image region center X", min_value=1, max_value=n_mls_x, value=54
                )
                start_ml = [crop_pos_y, crop_pos_x]
                start_coords = [sc * optical_info["pixels_per_ml"] for sc in start_ml]
                end_coords = [
                    sc + optical_info["n_micro_lenses"] * optical_info["pixels_per_ml"]
                ...
                st.session_state["azim_image_measured"] = azim_img_raw[
                    start_coords[0] : end_coords[0], start_coords[1] : end_coords[1]
                ]
            ...
            optical_info["volume_shape"][1] = st.slider(
                "Y-Z volume dimension", min_value=1, max_value=100, value=51
            )
            optical_info["volume_shape"][2] = optical_info["volume_shape"][1]
        else:
            optical_info["n_voxels_per_ml_volume"] = st.slider(
                "Number of voxels per microlens in volume space",
                min_value=1,
                max_value=21,
                value=1,
            )
            volume_type = st.selectbox(
                "Volume type", ["ellipsoid", "shell", "2ellipsoids", "single_voxel"], 3
            )
            st.subheader("Volume shape")
            optical_info["volume_shape"][0] = st.slider(
                "Axial volume dimension", min_value=1, max_value=50, value=5
            )
            # y will follow x if x is changed. x will not follow y if y is changed
            optical_info["volume_shape"][1] = st.slider(
                "Y-Z volume dimension", min_value=1, max_value=100, value=51
    ...
    with volume_container:
        if how_get_vol == "h5 upload":
            ...
            if h5file is not None:
                ...
                optical_info_volume["n_voxels_per_ml"] = optical_info_volume[
                    "n_voxels_per_ml_volume"
                ]
                ...
                st.session_state["my_volume"] = BirefringentVolume.init_from_file(
                    h5file, backend=backend, optical_info=optical_info_volume
                )
...
training_params = {
    ...
    "reg": [
        regularization_function1,
        regularization_function2,
...
if st.button("Reconstruct!"):
    ...
    estimated_volume_shape[0] = st.slider(
        "Axial volume dimension for estimated volume",
        min_value=1,
        max_value=50,
        value=5,
    )
    # y will follow x if x is changed. x will not follow y if y is changed
    estimated_volume_shape[1] = st.slider(
        "Y-Z volume dimension for estimated volume",
        min_value=1,
        max_value=100,
        value=31,
    )
    ...
    with torch.no_grad():
        if how_get_vol == "Upload experimental images":
            ...
            rays_higher_sampling = BirefringentRaytraceLFM(
                backend=backend, optical_info=optical_info_volume
            )
            rays_higher_sampling.compute_rays_geometry()
            # Perform same calculation with torch
            start_time = time.time()
            [ret_image_measured, azim_image_measured] = (
                rays_higher_sampling.ray_trace_through_volume(my_volume)
            )
    ...
    if volume_init_type == "upload" and h5file_init is not None:
        volume_estimation = BirefringentVolume.init_from_file(
            h5file_init, backend=backend, optical_info=optical_info
        )
    else:
        volume_estimation = BirefringentVolume(
            backend=backend,
            optical_info=optical_info,
    ...
    parameters = [
        ...
        {"params": trainable_parameters[1], "lr": training_params["lr"]},
    ]  # Delta_n
    ...
    for ep in tqdm(range(training_params["n_epochs"]), "Minimizing"):
        ...
        # Forward projection
        [ret_image_current, azim_image_current] = rays.ray_trace_through_volume(
            volume_estimation
        )
        ...
        with torch.no_grad():
            ...
            volume_estimation.optic_axis[
                :, torch.isnan(volume_estimation.optic_axis[0, :])
            ] = replacement_vecs
        ...
        if ep % 2 == 0:
            ...
            fig = plot_iteration_update(
                volume_2_projections(Delta_n_GT.unsqueeze(0))[0, 0]
                .detach()
                .cpu()
                .numpy(),
                ret_image_measured.detach().cpu().numpy(),
                azim_image_measured.detach().cpu().numpy(),
                volume_2_projections(volume_estimation.get_delta_n().unsqueeze(0))[0, 0]
                .detach()
                .cpu()
                .numpy(),
    ...
    st.download_button(
        ...
        volume_estimation.save_as_file(h5_file),
