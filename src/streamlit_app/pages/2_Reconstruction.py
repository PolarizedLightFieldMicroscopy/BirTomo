...
loss_function = optim_cols[0].selectbox(
    "Loss function", ["vonMisses", "vector", "L1_cos", "L1all"], 1
)
regularization_function1 = optim_cols[0].selectbox(
    "Volume regularization function 1", ["L1", "L2", "unit", "TV", "none"], 2
)
regularization_function2 = optim_cols[0].selectbox(
    "Volume regularization function 2", ["L1", "L2", "unit", "TV", "none"], 4
)
reg_weight1 = optim_cols[0].number_input(
    "Regularization weight 1", min_value=0.0, max_value=0.5, value=0.5
)
reg_weight2 = optim_cols[0].number_input(
    "Regularization weight 2", min_value=0.0, max_value=0.5, value=0.5
)
ret_azim_weight = optim_cols[0].number_input(
    "Retardance-Orientation weight", min_value=0.0, max_value=1.0, value=0.5
)
optim_cols[1].markdown("**Learning rate**")
learning_rate_delta_n = optim_cols[1].number_input(
    "Learning rate for the birefringence $\Delta n$",
    min_value=0.0,
    max_value=10.0,
    value=0.001,
    format="%0.5f",
)
learning_rate_optic_axis = optim_cols[1].number_input(
    "Learning rate for optic axis $\mathbf{a}$",
    min_value=0.0,
    max_value=10.0,
    value=0.001,
    format="%0.5f",
...
est_volume_init_type = optim_cols[1].selectbox(
    "Initial volume type", ["random", "upload"], 0
)
if est_volume_init_type == "upload":
    ...
    est_vol_shape[0] = optim_cols[1].slider(
    ...
    est_vol_shape[1] = optim_cols[1].slider(
        "Y-Z volume dimension", min_value=1, max_value=100, value=31
    )
...
with volume_container:
    ...
    if how_acquire == "External: experimental or synthetical":
        ...
        how_synthetic = forw_cols[0].radio(
            "Method of acquiring birefringent volume to image",
            ["Generate a new volume", "Upload h5 file"],
            index=0,
        )
        ...
        if how_synthetic == "Generate a new volume":
            ...
            volume_type = forw_cols[1].selectbox(
                "Volume type", ["ellipsoid", "shell", "single_voxel"], 2
            )  # 2ellipsoid not working
            forw_cols[1].write("*Volume shape*")
            optical_info["volume_shape"][0] = forw_cols[1].slider(
                "Axial volume dimension of GT", min_value=1, max_value=50, value=5
            )
            # y will follow x if x is changed. x will not follow y if y is changed
            optical_info["volume_shape"][1] = forw_cols[1].slider(
                "Y-Z volume dimension of GT", min_value=1, max_value=100, value=51
            )
            optical_info["volume_shape"][2] = optical_info["volume_shape"][1]
            shift_from_center = forw_cols[1].slider(
            ...
            h5file = forw_cols[1].file_uploader(
                "Upload a ground truth volume", type=["h5"]
            )
...
if st.button("**Calculate retardance and orientation images!**", key="calc1"):
    with torch.no_grad():
        ...
        [ret_image, azim_image], execution_time = forward_propagate(
            rays, st.session_state.GT
        )
        # st.text(f'Execution time in seconds with backend {backend}: ' + str(execution_time))
        st.success(
            f"Geometric ray tracing was successful in {execution_time:.2f} secs!",
            icon="✅",
        )
...
if st.button(
    "**Calculate retardance and orientation images from classes!**", key="calc2"
):
...
if "ret_image" in st.session_state:
    ...
    my_fig = plot_retardance_orientation(
        output_ret_image, output_azim_image, azimuth_plot_type
    )
    st.pyplot(my_fig)
...
if st.button("**Reconstruct birefringent volume!**"):
    ...
    if est_volume_init_type == "upload" and est_h5file_init is not None:
        volume_estimation = BirefringentVolume.init_from_file(
            est_h5file_init, backend=backend, optical_info=optical_info
        )
    else:
...
if "vol_est" in st.session_state:
    ...
    st.session_state["fig_est"] = st.session_state["vol_est"].plot_lines_plotly(
        delta_n_ths=st.session_state["vol_threshold"]
    )
    ...
    st.write(
        "Downloaded estimated retardance and orientation images may not be \
             saved in a way that another program can read the images."
    )
    st.download_button(
        "Download estimated retardance",
        st.session_state["ret_est"].tobytes(),
        mime="image/jpeg",
        file_name="retardance_estimated.jpeg",
    )
    st.download_button(
        "Download estimated orientation",
