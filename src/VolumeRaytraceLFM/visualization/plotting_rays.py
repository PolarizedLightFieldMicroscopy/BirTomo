...
def plot_ray_path(
    ...
    try:
        ...
        print(
            "Error in plot_ray_path: optical_config should be a dictionary containing the required variables..."
        )
        return
    ...
    if use_matplotlib:
        ...
        ax.scatter(
            (z_indices + offset) * dxy,
            (y_indices + offset) * dxy,
            (x_indices + offset) * dxy,
            s=dxy,
        )
        ...
        for vix in range(len(z_indices)):
            ...
            offset = 0
            voxel_coord_low = [
                (z_indices[vix] + offset) * dz,
                (y_indices[vix] + offset) * dxy,
                (x_indices[vix] + offset) * dxy,
            ]
            offset = 1
            voxel_coord_top = [
                (z_indices[vix] + offset) * dz,
...
def plot_rays_at_sample(
    ray_entry, ray_exit, colormap="inferno", optical_config=None, use_matplotlib=False
):
    ...
    if use_matplotlib:
        ...
        if optical_config is not None:
            ...
            mla_sample_pitch = (
                optical_config.mla_config.pitch / optical_config.PSF_config.M
            )
            x = [
            ...
            ax.add_collection3d(Poly3DCollection(verts, alpha=0.20))
