...
def apply_loss_function_and_reg(
    loss_type,
    reg_types,
    retardance_measurement,
    ...
    if loss_type == "vonMisses":
        ...
        data_term = (
            1 - ret_orie_weight
        ) * retardance_loss + ret_orie_weight * angular_loss
    # Vector difference
    if loss_type == "vector":
        co_gt, ca_gt = ret_meas * torch.cos(orien_meas), ret_meas * torch.sin(
            orien_meas
        )
        co_pred, ca_pred = ret_est * torch.cos(orien_est), ret_est * torch.sin(
            orien_est
        )
        data_term = ((co_gt - co_pred) ** 2 + (ca_gt - ca_pred) ** 2).mean()
    elif loss_type == "L1_cos":
        data_term = (1 - ret_orie_weight) * F.l1_loss(
            ret_meas, ret_est
        ...
        data_term = (ret_meas - ret_est).abs().mean() + (
            2 * (1 - torch.cos(orien_meas - orien_est)) * azimuth_damp_mask
        ).mean()
            ...
            # Unit length regularizer
                ...
                regularization_term = (
                    (
                        1
                        - (
                            volume_estimate.optic_axis[0, ...] ** 2
                            + volume_estimate.optic_axis[1, ...] ** 2
                            + volume_estimate.optic_axis[2, ...] ** 2
                        )
                    )
                    .abs()
                    .mean()
                ...
                regularization_term = torch.zeros([1], device=ret_meas.device)
...
def weighted_local_cosine_similarity_loss(optic_axis, delta_n):
    ...
    assert torch.all(
        (normalized_delta_n >= -0.5) & (normalized_delta_n <= 0.5)
    ), err_message
    ...
    for i in range(1, 4):  # Iterate over D, H, W dimensions
        ...
        weighted_cos_sim = (
            cos_sim * (normalized_delta_n.abs() + rolled_delta_n.abs()) / 2
        )

        # Create the valid mask to avoid boundary elements
        valid_mask = torch.ones_like(weighted_cos_sim, dtype=torch.bool)
        valid_mask.index_fill_(
            i - 1, torch.tensor([weighted_cos_sim.size(i - 1) - 1]), False
        )
