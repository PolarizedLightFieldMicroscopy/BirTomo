...
def instance_iterable_inputs_for_image_generation(
    self, mla_indices, offsets, volume_in, intensity
):
    def get_input(i):
        ml_jj_idx, ml_ii_idx = mla_indices[i]
        current_offset = offsets[i]
        img_list = self.generate_images(
            volume_in, current_offset, intensity, mla_index=(ml_jj_idx, ml_ii_idx)
        )
        return img_list
...
def process_row(
    self, ml_ii_idx, row_iterable, n_ml_half, odd_mla_shift, volume_in, intensity
):
...
def make_process_row_method(instance):
    def process_row(
        ml_ii_idx, row_iterable, n_ml_half, odd_mla_shift, volume_in, intensity
    ):
        full_img_row_list = [None] * 5
        for ml_jj_idx, ml_jj in enumerate(range(-n_ml_half, n_ml_half + odd_mla_shift)):
            current_offset = instance._calculate_current_offset(
                row_iterable[ml_ii_idx],
                ml_jj,
                instance.n_voxels_per_ml,
                instance.n_micro_lenses,
            )
            img_list = instance.generate_images(
                volume_in, current_offset, intensity, mla_index=(ml_jj_idx, ml_ii_idx)
            )
...
full_img_list_array = torch.empty(
    (2, n_micro_lenses * num_pix, n_micro_lenses * num_pix)
)
ml_ii_tensor = torch.arange(-n_ml_half, n_ml_half + odd_mla_shift)
ml_jj_tensor = torch.arange(-n_ml_half, n_ml_half + odd_mla_shift)
mla_index_tensor = torch.stack(
    torch.meshgrid(torch.arange(n_micro_lenses), torch.arange(n_micro_lenses)), dim=-1
)
...
for ml_ii_idx, ml_ii in enumerate(ml_ii_tensor):
    for ml_jj_idx, ml_jj in enumerate(ml_jj_tensor):
        current_offset = self._calculate_current_offset(
            ml_ii.item(), ml_jj.item(), n_voxels_per_ml, n_micro_lenses
        )
...
lenslets_reshaped = img_list_all_lenslets.view(
    n_micro_lenses, n_micro_lenses, 2, num_pix, num_pix
)
...
for i in range(n_micro_lenses):
    for j in range(n_micro_lenses):
        ...
        full_img_list_array[:, start_y:end_y, start_x:end_x] = lenslets_reshaped[
            j, i, :, :, :
        ]
...
if parallelize_pool:
    get_input = self.instance_iterable_inputs_for_image_generation(
        mla_indices, offsets, volume_in, intensity
    )
...
if pool_method_attempt:
    with torch.multiprocessing.Pool() as pool:
        ...
        results = [
            ...
            for ml_ii_idx in range(len(row_iterable))
        ]
