...
def normalize_retardance(ret_image_polscope, ret_ceiling, wavelength=550):
    ...
    assert (
        np.min(ret_image) >= 0
    ), f"Minimun retardance value is below zero: Min = {np.min(ret_image):.3f}"
    assert (
        np.max(ret_image) <= np.pi
    ), f"Maximum retardance value exceeded π: Max = {np.max(ret_image):.3f}"
...
def normalize_azimuth(azim_image_polscope):
    ...
    assert (
        np.min(azim_image) >= 0
    ), f"Minimun azimuth value is below zero: Min = {np.min(azim_image):.3f}"
    assert (
        np.max(azim_image) <= np.pi
    ), f"Maximum azimuth value exceeded π: Max = {np.max(azim_image):.3f}"
...
def prepare_ret_azim_images(retardance_path, azimuth_path, ret_ceiling, wavelength_um):
    ...
    ret_image_meas = normalize_retardance(
        ret_polscope, ret_ceiling, wavelength=wavelength_nm
    )
